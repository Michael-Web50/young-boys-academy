  ...
  return (
    <main className="min-h-screen bg-brand-white">
      ...
      <section className="pt-32 pb-16 px-4 bg-brand-black text-center">
        ...
        <p className="text-xl text-brand-white/80 max-w-3xl mx-auto">Join us in nurturing the next generation of football champions. Your support makes a difference.</p>
      </section>
      ...
      <section className="py-16 px-4 bg-brand-lightGray">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-3xl font-extrabold text-brand-black text-center mb-4">Sponsorship <span className="text-brand-yellow">Packages</span></h2>
          <p className="text-center text-brand-darkGray mb-12 max-w-2xl mx-auto">Choose the sponsorship tier that best fits your organization's goals and budget.</p>
          <div className="grid md:grid-cols-3 gap-8">
            {sponsorshipTiers.map((tier, index) => (
              <div key={index} className={`bg-brand-white p-8 rounded-2xl shadow-xl border-2 ${tier.name === "Gold" ? "border-brand-yellow transform scale-105" : "border-transparent"}`}>
                {tier.name === "Gold" && <div className="bg-brand-yellow text-brand-black text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full inline-block mb-4">Most Popular</div>}
                ...
                <ul className="space-y-3 mb-8">
                  {tier.benefits.map((benefit, i) => (
                    <li key={i} className="flex items-start gap-2"><CheckCircle className="w-5 h-5 text-brand-yellow flex-shrink-0 mt-0.5" /><span className="text-sm text-brand-darkGray">{benefit}</span></li>
                  ))}
                </ul>
                <a href="#application-form" className="block w-full text-center bg-brand-black text-brand-white py-3 rounded-lg font-bold hover:bg-brand-yellow hover:text-brand-black transition-colors">Apply Now</a>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="application-form" className="py-16 px-4">
        <div className="max-w-3xl mx-auto">
          <h2 className="text-3xl font-extrabold text-brand-black text-center mb-4">Sponsorship <span className="text-brand-yellow">Application</span></h2>
          <p className="text-center text-brand-darkGray mb-12">Fill out the form below and our partnership team will contact you within 48 hours.</p>

          {submitted && (
            <div className="mb-8 bg-green-50 border-2 border-green-300 rounded-xl p-6 flex items-start gap-4">
              <CheckCircle className="w-8 h-8 text-green-600 flex-shrink-0" />
              <div><h3 className="font-bold text-green-900 mb-1">Application Sent Successfully!</h3><p className="text-green-800 text-sm">Thank you for your interest. We have received your application and will contact you shortly.</p></div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="bg-brand-lightGray p-8 rounded-2xl border border-brand-yellow/20">
            ...
            <div className="mb-6">
              ...
              <textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} rows={4} className="w-full px-4 py-3 rounded-lg bg-brand-white border border-brand-yellow/30 text-brand-black focus:border-brand-yellow focus:outline-none resize-none" placeholder="Tell us about your organization..." />
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full bg-brand-yellow text-brand-black py-4 rounded-lg font-bold text-lg hover:bg-yellow-400 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
              {isSubmitting ? <><Loader2 className="animate-spin" size={20} /> Sending...</> : <><Send size={20} /> Submit Application</>}
            </button>
          </form>
        </div>
      </section>
