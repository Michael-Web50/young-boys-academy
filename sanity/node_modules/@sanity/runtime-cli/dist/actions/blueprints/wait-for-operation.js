...
export async function waitForOperation(options) {
    ...
    try {
        ...
        let nextIdleSpinnerUpdateAt = IDLE_SPINNER_DELAY_MS;
        ...
        logStreamCleanup = setupLogPolling({
            ...
            onActivity: () => {
                lastLogAt = Date.now();
                nextIdleSpinnerUpdateAt = IDLE_SPINNER_DELAY_MS;
        ...
        while (true) {
            ...
            if (idleFor >= nextIdleSpinnerUpdateAt) {
                const elapsedIdleMs = IDLE_SPINNER_DELAY_MS +
                    Math.floor((idleFor - IDLE_SPINNER_DELAY_MS) / IDLE_SPINNER_UPDATE_INTERVAL_MS) *
                        IDLE_SPINNER_UPDATE_INTERVAL_MS;
                const elapsedIdleSeconds = elapsedIdleMs / 1000;
                const text = `No new activity for ${elapsedIdleSeconds} seconds. The ${progressNoun} is still running on Sanity servers. You can safely exit and check status later with \`npx ${bin} blueprints info\`.`;
                if (idleSpinner)
