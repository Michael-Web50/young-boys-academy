...
function normalizeResourceOptions(options, resources, contextResource, contextPerspective) {
	let { resourceName, sourceName, source, projectId, dataset, ...rest } = options, effectiveResourceName = resourceName ?? sourceName, effectiveResource = options.resource ?? source;
	...
	if (effectiveResource) resolvedResource = effectiveResource;
...
function useResourceIdFromDocumentHandle(documentHandle) {
	let $ = c(3), { resource } = useNormalizedResourceOptions(documentHandle), resourceId = "", resourceType;
...
function useDispatchIntent(params) {
	let { action, intentId, documentHandle, parameters } = params, { sendMessage } = useWindowConnection({
		name: SDK_NODE_NAME,
		connectTo: SDK_CHANNEL_NAME
	}), resource = useResourceIdFromDocumentHandle(documentHandle);
	return { dispatchIntent: useCallback(() => {
		try {
...
* ```
*/
function useManageFavorite({ documentId, documentType, projectId: paramProjectId, dataset: paramDataset, resourceId: paramResourceId, resourceType, schemaName }) {
	let { fetch } = useWindowConnection({
		name: SDK_NODE_NAME,
		connectTo: SDK_CHANNEL_NAME
	...
	let context = useMemo(() => ({
		...
		resourceType,
		schemaName
	]), favoriteState = getFavoritesState(instance, context), isFavorited = useSyncExternalStore(favoriteState.subscribe, favoriteState.getCurrent)?.isFavorited ?? !1, handleFavoriteAction = useCallback(async (action) => {
...
function useQuery(options) {
	...
	let normalized = useNormalizedResourceOptions(options), [isPending, startTransition] = useTransition(), queryKey = getQueryKey(normalized), [deferredQueryKey, setDeferredQueryKey] = useState(queryKey), ref = useRef(new AbortController());
	useEffect(() => {
...
export { AuthBoundary, ComlinkTokenRefreshProvider, REACT_SDK_VERSION, ResourceProvider, SDKProvider, SDKStudioContext, SanityApp, SanityInstanceProvider, renderSanityApp, useActiveReleases, useAgentGenerate, useAgentPatch, useAgentPrompt, useAgentResourceContext, useAgentTransform, useAgentTranslate, useAllReleases, useApplyDocumentActions, useApplyReleaseActions, useAuthState, useAuthToken, useClient, useCommentActions, useCommentThreads, useComments, useCreateDocument, useCurrentUser, useDashboardNavigate, useDashboardOrganizationId, useDatasets, useDispatchIntent, useDocument, useDocumentEvent, useDocumentPermissions, useDocumentPreview, useDocumentProjection, useDocumentSyncStatus, useDocuments, useEditDocument, useFrameConnection, useHandleAuthCallback, useLogOut, useLoginUrl, useManageFavorite, useNavigateToStudioDocument, useOrganization, useOrganizations, usePaginatedDocuments, usePerspective, usePresence, usePresenceForDocument, useProject, useProjects, useQuery, useRecordDocumentHistoryEvent, useReportPresence, useResource, useSanityInstance, useStudioWorkspacesByProjectIdDataset, useUser, useUsers, useVerifyOrgProjects, useWindowConnection, useWindowTitle };
