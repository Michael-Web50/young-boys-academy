import { A as defineStore, C as REQUEST_TAG_PREFIX, D as bindActionByResource, E as createStateSourceAction, F as isDatasetSource, I as isMediaLibraryResource, L as isMediaLibrarySource, M as isCanvasResource, N as isCanvasSource, O as bindActionByResourceAndPerspective, P as isDatasetResource, S as DEFAULT_API_VERSION, _ as getDefaultLocation, a as getCurrentUserState, b as AuthStateType, c as getLoginUrlState, d as createLoggedInAuthState, f as getAuthCode, g as getClientErrorApiType, h as getClientErrorApiDescription, i as getAuthState, j as isReleasePerspective, k as bindActionGlobally, l as getTokenState, m as getClientErrorApiBody, o as getDashboardOrganizationId, p as getCleanedUrl, s as getIsInDashboardState, t as authStore, u as setAuthToken, v as getTokenFromLocation, w as getAuthLogger, x as isStudioConfig, y as isProjectUserNotFoundClientError } from "./authStore-CWTAuieY.js";
import { A as addComment, B as normalizeComment, C as getCommentThreadsState, D as resolveCommentThreads, F as removeCommentById, G as COMMENTS_API_VERSION, H as assertDatasetResource, I as restoreComments, K as setCleanupTimeout, L as rollbackCommentUpdate, M as clearPendingTransaction, N as getCommentsKey, O as resolveComments, P as receiveComment, R as setCommentCreateError, S as commentsStore, T as getCommentsState, U as getAddonDatasetState, V as toCommentFieldPath, W as provisionAddonDataset, _ as setUsersData, a as getQueryState, b as PROJECT_API_VERSION, c as getPerspectiveState, d as addSubscription, f as cancelRequest, g as removeSubscription, h as parseUsersKey, i as getQueryKey, j as applyCommentUpdate, k as toCommentsKeyParts, l as getActiveReleasesState, m as initializeRequest, n as transformProjectionToPreview, o as parseQueryKey, p as getUsersKey, q as insecureRandomId, r as PREVIEW_PROJECTION, s as resolveQuery, t as createGroqSearchFilter, u as getAllReleasesState, v as setUsersError, x as USERS_STATE_CLEAR_DELAY, y as updateLastLoadMoreRequest, z as setPendingTransaction } from "./createGroqSearchFilter-B0NdA-EP.js";
import { n as configureLogging$1, r as createLogger } from "./getEnv-76AjZd_f.js";
import { i as isDeepEqual, n as getClient, o as omitProperty, r as getClientState, s as pickProperties } from "./clientStore-CMzJ7XhZ.js";
import { t as CORE_SDK_VERSION } from "./version-ByqNiWrK.js";
import { ClientError, CorsOriginError } from "@sanity/client";
import { EMPTY, NEVER, Observable, Subject, Subscription, asapScheduler, auditTime, catchError, combineLatest, concat, concatMap, debounceTime, defer, distinctUntilChanged, filter, first, firstValueFrom, from, fromEvent, groupBy, map, merge, mergeMap, of, pairwise, partition, race, retry, share, shareReplay, skip, startWith, switchMap, take, takeUntil, tap, throttle, throwError, timer, withLatestFrom } from "rxjs";
import { isKeySegment, isKeyedObject } from "@sanity/types";
import { catchError as catchError$1, delay, distinctUntilChanged as distinctUntilChanged$1, filter as filter$1, first as first$1, groupBy as groupBy$1, map as map$1, mergeMap as mergeMap$1, pairwise as pairwise$1, retry as retry$1, scan, share as share$1, startWith as startWith$1, switchMap as switchMap$1, tap as tap$1 } from "rxjs/operators";
...
function createSanityInstance(config = {}) {
	...
	let instance = {
		...
		createChild: (next) => {
			...
			let child = Object.assign(createSanityInstance({
				...config,
				...next,
				...config.auth === next.auth ? config.auth : config.auth && next.auth && { auth: {
					...config.auth,
					...next.auth
				} }
			}), { getParent: () => instance });
		...
		match: (targetConfig) => {
			if (Object.entries(pickProperties(targetConfig, [
				"auth",
				"projectId",
				"dataset"
			])).every(([key, value]) => config[key] === value)) return instance;
...
*/
function createFetcherStore({ name, fetcher: getObservable, getKey, fetchThrottleInternal = 1e3, stateExpirationDelay = 5e3 }) {
	let store = defineStore({
		name,
		getInitialState: () => ({ stateByParams: {} }),
		initialize: (context) => {
			let subscription = subscribeToSubscriptionsAndFetch(context);
			return () => subscription.unsubscribe();
		}
	}), subscribeToSubscriptionsAndFetch = ({ state }) => state.observable.pipe(switchMap$1((s) => {
		let entries = Object.entries(s.stateByParams);
		return entries.length > 0 ? from(entries) : EMPTY;
	}), groupBy$1(([key]) => key), mergeMap$1((group$) => group$.pipe(startWith$1([group$.key, void 0]), pairwise$1(), filter$1(([[, prevEntry], [, currEntry]]) => {
		let prevSubs = prevEntry?.subscriptions ?? [];
		return (currEntry?.subscriptions ?? []).length > prevSubs.length;
	}), map$1(([, [, currEntry]]) => currEntry), filter$1((entry) => {
		let lastFetch = entry?.lastFetchInitiatedAt;
		return !lastFetch || Date.now() - new Date(lastFetch).getTime() >= fetchThrottleInternal;
	}), switchMap$1((entry) => entry ? (state.set("setLastFetchInitiatedAt", (prev) => ({ stateByParams: {
		...prev.stateByParams,
		[entry.key]: {
			...entry,
	...
	} })), getObservable(entry.instance)(...entry.params).pipe(delay(0, asapScheduler), tap$1((data) => state.set("setData", (prev) => ({ stateByParams: {
		...prev.stateByParams,
		[entry.key]: {
	...
	} }))), catchError$1((error) => (state.set("setError", (prev) => ({ stateByParams: {
		...prev.stateByParams,
		[entry.key]: {
			...entry,
			...prev.stateByParams[entry.key],
	...
	} })), EMPTY)))) : EMPTY)))).subscribe({ error: (error) => state.set("setError", { error }) }), getState = bindActionGlobally(store, createStateSourceAction({
		selector: ({ instance, state: { stateByParams, error } }, ...params) => {
			if (error) throw error;
			let entry = stateByParams[getKey(instance, ...params)];
			if (entry?.error) throw entry.error;
			return entry?.data;
		},
		onSubscribe: ({ instance, state }, ...params) => {
			let subscriptionId = insecureRandomId(), key = getKey(instance, ...params);
			return state.set("addSubscription", (prev) => ({ stateByParams: {
				...prev.stateByParams,
				[key]: {
				...
				setCleanupTimeout(() => {
					state.set("removeSubscription", (prev) => {
						let entry = prev.stateByParams[key];
						if (!entry) return prev;
						let newSubs = (entry.subscriptions || []).filter((id) => id !== subscriptionId);
						return newSubs.length === 0 ? { stateByParams: omitProperty(prev.stateByParams, key) } : { stateByParams: {
							...prev.stateByParams,
							[key]: {
								...entry,
	...
	return {
		getState,
		resolveState: bindActionGlobally(store, ({ instance }, ...params) => firstValueFrom(getState(instance, ...params).observable.pipe(first$1((i) => i !== void 0))))
	};
}
...
function getProjectCacheKey(instance, options) {
	let projectId = resolveProjectId(instance, options), { includeMembers, includeFeatures } = normalizeProjectOptions(options);
...
const project = createFetcherStore({
	...
	fetcher: (instance) => (options) => {
		let projectId = resolveProjectId(instance, options);
		return getClientState(instance, {
			apiVersion: "v2025-02-19",
			scope: "global"
		}).observable.pipe(switchMap((client) => {
			let normalized = normalizeProjectOptions(options), query = Object.fromEntries(Object.entries(normalized).filter(([, value]) => value !== void 0).map(([key, value]) => [key, String(value)]));
...
}), getProjectState = project.getState, resolveProject = project.resolveState, getDashboardOrganizationId$1 = bindActionGlobally(authStore, createStateSourceAction(({ state: { dashboardContext } }) => dashboardContext?.orgId));
/**
...
function observeOrganizationVerificationState(instance, projectIds) {
	let dashboardOrgId$ = getDashboardOrganizationId$1(instance).observable.pipe(distinctUntilChanged()), projectOrgIdObservables = projectIds.map((id) => getProjectState(instance, { projectId: id }).observable.pipe(map((project) => ({
		projectId: id,
...
function revertOutgoingTransaction(prev) {
	...
	return {
		...
		applied: nextApplied,
		outgoing: void 0,
		documentStates: Object.fromEntries(Object.entries(base.documentStates).filter((e) => !!e[1]).map(([documentId, { unverifiedRevisions = {}, local, ...documentState }]) => [documentId, {
			...documentState,
...
function removeSubscriptionIdFromDocument(prev, documentId, subscriptionId) {
	let prevDocState = prev.documentStates?.[documentId], subscriptions = (prevDocState?.subscriptions ?? []).filter((id) => id !== subscriptionId);
...
const favorites = createFetcherStore({
	...
	fetcher: (instance) => (context) => {
		...
		return nodeStateSource.observable.pipe(filter((nodeState) => !!nodeState), shareReplay(1), switchMap((nodeState) => {
...
function getOrganizationCacheKey(_instance, options) {
	let organizationId = resolveOrganizationId(options), { includeMembers, includeFeatures } = normalizeOrganizationOptions$1(options);
...
const organization = createFetcherStore({
	...
	fetcher: (instance) => (options) => {
		let organizationId = resolveOrganizationId(options);
		return getClientState(instance, {
			apiVersion: "v2025-02-19",
			scope: "global"
		}).observable.pipe(switchMap((client) => {
			let normalized = normalizeOrganizationOptions$1(options), query = Object.fromEntries(Object.entries(normalized).filter(([, value]) => value !== void 0).map(([key, value]) => [key, String(value)]));
			return client.observable.request({
...
}), getOrganizationState = organization.getState, resolveOrganization = organization.resolveState;
...
function getOrganizationsCacheKey(_instance, options) {
	let { includeMembers, includeFeatures, includeImplicitMemberships } = normalizeOrganizationOptions(options);
...
const organizations = createFetcherStore({
	...
	fetcher: (instance) => (options) => getClientState(instance, {
		...
		let normalized = normalizeOrganizationOptions(options), query = Object.fromEntries(Object.entries(normalized).filter(([, value]) => value !== void 0).map(([key, value]) => [key, String(value)]));
		return client.observable.request({
			uri: "/organizations",
			query,
...
function getPreviewState(instance, options) {
	let projectionState = getProjectionState(instance, {
		...options,
		projection: PREVIEW_PROJECTION
	}), transformResult = (current) => !current || current.data === null ? {
		data: null,
		isPending: current?.isPending ?? !1
	} : {
...
async function resolvePreview(instance, options) {
	let projectionResult = await resolveProjection(instance, {
		...options,
		projection: PREVIEW_PROJECTION
	});
	return projectionResult.data ? {
		data: transformProjectionToPreview(instance, projectionResult.data, options.resource),
...
const projects = createFetcherStore({
	...
	fetcher: (instance) => (options) => getClientState(instance, {
		...
		let normalized = normalizeProjectsOptions(options), query = Object.fromEntries(Object.entries(normalized).filter(([, value]) => value !== void 0).map(([key, value]) => [key, String(value)]));
		return client.observable.request({
			uri: "/projects",
			query,
			tag: "projects.get"
...
function defineIntent(intent) {
	if (!intent.id) throw Error("Intent must have an id");
	if (!intent.action) throw Error("Intent must have an action");
	if (!intent.title) throw Error("Intent must have a title");
	if (!Array.isArray(intent.filters)) throw Error("Intent must have a filters array");
	if (intent.filters.length === 0) throw Error("Intent must have at least one filter. If you want to match everything, use {types: ['*']}");
	return intent.filters.forEach((filter, index) => {
...
* @internal
*/
function validateFilter(filter, index) {
	let filterContext = `Filter at index ${index}`;
	if (!filter || typeof filter != "object") throw Error(`${filterContext} must be an object`);
	if (filter.types === void 0) throw Error(`${filterContext} must have a types property. Use ['*'] to match all document types.`);
	if (filter.projectId !== void 0) {
		if (typeof filter.projectId != "string") throw Error(`${filterContext}: projectId must be a string`);
		if (filter.projectId.trim() === "") throw Error(`${filterContext}: projectId cannot be empty`);
	}
	if (filter.dataset !== void 0) {
		if (typeof filter.dataset != "string") throw Error(`${filterContext}: dataset must be a string`);
		if (filter.dataset.trim() === "") throw Error(`${filterContext}: dataset cannot be empty`);
		if (filter.projectId === void 0) throw Error(`${filterContext}: dataset cannot be specified without projectId`);
	}
	if (!Array.isArray(filter.types)) throw Error(`${filterContext}: types must be an array`);
	if (filter.types.length === 0) throw Error(`${filterContext}: types array cannot be empty`);
	if (filter.types.forEach((type, typeIndex) => {
		if (typeof type != "string") throw Error(`${filterContext}: types[${typeIndex}] must be a string`);
		if (type.trim() === "") throw Error(`${filterContext}: types[${typeIndex}] cannot be empty`);
	}), filter.types.includes("*") && filter.types.length > 1) throw Error(`${filterContext}: when using wildcard '*', it must be the only type in the array`);
}
/**
* @public
