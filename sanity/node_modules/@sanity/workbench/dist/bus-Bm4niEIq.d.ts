import { V as RemoteModule, n as ApplicationList, t as Application } from "./application-list-1MWg2hXK.js";
...
interface WorkbenchTopics {
  /**
   * The application the URL currently puts in the foreground; `null` when none
   * is — including before the location and the application list are known, and
   * once the session ends. Seeded `null`. In-app navigation stays silent, but a
   * resync republishes the rebuilt instance even when the foregrounded app's id
   * is unchanged. Published by the applications machine.
   *
   * Carries the live `Application` instance (its getters and methods), so this
   * topic is same-window only — it is not bridgeable (see `topics.test.ts`).
   */
  "applications.foreground": StateTopicDef<Application | null>;
  /**
   * The applications available in this workbench, as an `ApplicationList` —
   ...
   *
   * Carries the live `ApplicationList` instance (its lookup methods), so this
   * topic is same-window only — it is not bridgeable (see `topics.test.ts`).
   */
  "applications.list": StateTopicDef<TopicResult<ApplicationList> | null>;
  /**
   ...
   * current location replies `ok` without navigating.
   *
   * `{ appId }` goes to that app's root, `{ appId, path }` deep-links inside it.
   * Without an `appId`, `path` names a workbench-level page (empty payload goes
   * home), so a `navigation.location` value echoes back as-is. `replace` swaps
   * the current history entry instead of pushing one.
   *
   * `ok: false` reasons:
   * - `not-navigable` — panel-only, a blank id, or the path is invalid or escapes the app's base
   ...
   * Published synchronously at boot by the system-preferences machine.
   */
  "preferences.dock-locked": StateTopicDef<SystemPreferencesContext["dockLocked"]>;
  /**
 ...
 * @public
 */
type StateTopic = StateTopicsOf<Topics>;
/**
...
export { disconnectApp as a, NavigationTarget as c, StateTopic as d, TopicResult as f, SystemPreferencesEvent as g, SystemPreferencesContext as h, defineStateTopics as i, PayloadOf as l, SystemPreferencesAdapter as m, StateSource as n, os as o, ValueOf as p, connect as r, NavigationLocation as s, ConnectOptions as t, ReplyOf as u };
//# sourceMappingURL=bus-Bm4niEIq.d.ts.map